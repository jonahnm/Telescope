...
static inline u64 phystokv(struct kfd* kfd, u64 pa);
static inline u64 vtophys(struct kfd* kfd, u64 va);

static inline void perf_kread(struct kfd* kfd, u64 kaddr, void* uaddr, u64 size)
{
...
static inline void perf_kwrite(struct kfd* kfd, void* uaddr, u64 kaddr, u64 size)
{
...
static inline void perf_init(struct kfd* kfd)
{
...
static inline void perf_run(struct kfd* kfd)
{
    if (!kern_versions[kfd->info.env.vid].perf_supported) {
        return;
    }
...
static inline void perf_free(struct kfd* kfd)
{
    if (!kern_versions[kfd->info.env.vid].perf_supported) {
        return;
    }
