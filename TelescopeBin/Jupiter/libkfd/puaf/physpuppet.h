...
static const u64 physpuppet_vmne_size = pages(2) + 1;
static const u64 physpuppet_vme_offset = pages(1);
static const u64 physpuppet_vme_size = pages(2);

static inline void physpuppet_init(struct kfd* kfd)
{
    /*
     * Nothing to do.
     */
...
static inline void physpuppet_run(struct kfd* kfd)
{
...
static inline void physpuppet_cleanup(struct kfd* kfd)
{
...
static inline void physpuppet_free(struct kfd* kfd)
{
    u64 kread_page_uaddr = trunc_page(kfd->kread.krkw_object_uaddr);
    u64 kwrite_page_uaddr = trunc_page(kfd->kwrite.krkw_object_uaddr);
